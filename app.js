...
// Bonus 4 (NO toLocaleString, NO Math.round)
function bonus4() {
  clearGrid();
  const grid = document.getElementById("employeesGrid");
  ...
  const average = totalSalary / engineeringEmps.length;

  let html = "";
  html += `
    <div class="result-box">
      <h2>Bonus 4</h2>
      <h3 style="font-size: 2.5em; margin: 15px 0;">$${totalSalary}</h3>
      <p>Engineering Department Total Salary</p>
      <p style="font-size: 1em; margin-top: 10px; color: #ccc;">
        ${engineeringEmps.length} employees | Average: $${average}
      </p>
    </div>
  `;
